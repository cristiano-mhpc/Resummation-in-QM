...
const realtype nu = realtype(1)/realtype(5);

vector<realtype> term(d+1);
vector<realtype> factor( d+1 );
vector<realtype> factor1( 2*d + 1 );
...
int main()
{
    ...
    int share = (int) ( floor((d + 1)/ num_process) ); // 3 is the number of process
    ...
    {
        factor1(i) = pow( realtype(2), realtype(i+1)-nu) * boost::math::tgamma(realtype(i+1)-nu);
    }
    ...
    if (world.rank() < num_process - 1  && world.rank() != 0)
    {
        ...
        for (n = 0; n < P1.size1() ; ++n)
        {
            for ( k = 0; k < d+1; ++k )
            {
                term(k) = factor1( k + n + (world.rank() * share) )* factor2( k );
            }
    ...
    if (world.rank() == num_process - 1)
    {
        ...
        for (n = 0; n < P1.size1(); ++n)
        {
            for (k=0; k < d+1; ++k)
            {
                term(k) = factor1( k + n + ( (num_process - 1)  * share) )* factor2( k );
            }

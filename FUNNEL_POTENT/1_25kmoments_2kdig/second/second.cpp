...
int main()
{
    ...
    int share = (int) ( floor( ( d  ) / num_process ) );
    ...
    {
        factor(i) = realtype(1)/ boost::math::factorial < realtype > ( d - i ); //factorial < realtype >
    ...
    if ( world.rank() == 0 )
    {
        ...
        {
            vector<realtype> second1; //for storing the first1's from other processes
            ms1.push_back( second1 );
        }
        ...
        {
            //vector<realtype> term(k+1); we dont want repeated allocation
            for(int l = 0; l < index(k)+1; ++l)
            {
                term(l) = factor2(l) * factor1(d - index(k) + l); //tagal nagets to
            }

            for(int m = index(k) + 1; m < d+1; ++m)
            {
                term1(m) = inner_prod( project(term, range(0, index(k) + 1 ) ), project( factor, range ( d-m , d - m + index(k) + 1) ) );
            }

            second( k ) = inner_prod( project ( constants, range (index(k) + 1, d+1) ), project ( term1 , range ( index(k) + 1, d + 1 ) ) );
        }

        // receive all the second1's and indices and store them in elements of ms1[i] and ms2[i], respectively.
        for ( i = 0; i < num_process - 1 ; ++i )
        {
        ...
        outfile1.open("/scratch3/chris.tica/FUNNEL/1_25k_moments_2kdig/results/SECOND.txt",std::ios_base::out);
        ...
        for ( i = 0; i < omegas; ++i )
        {
            ...
            for (k = 0; k < index.size(); ++k)
            {
                total( index(k) ) = pow( omega(i), realtype( index(k) ) ) * second(k);
            }

            secondter(i) = sum(total);
            outfile1 << std::scientific << ( -sine )*secondter(i) << std::endl;
        }
    ...
    if ( world.rank() < LO  && world.rank() > 0  )
    {
        ...
        vector<realtype> second1( index.size() );
        for (int k = 0; k < index.size(); ++k)
        {
            //vector<realtype> term(k+1); we dont want repeated allocation
            for(int l = 0; l < index(k)+1; ++l)
            ...
            second1( k ) = inner_prod( project ( constants, range (index(k) + 1, d+1) ), project ( term1 , range ( index(k) + 1, d + 1 ) ) );
        }

        world.send( 0 ,15 , second1 );//send the computed values for the assigned k's

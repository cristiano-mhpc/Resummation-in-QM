...
int main()
{
    ...
    if ( world.rank() == 0 )
    {
        ...
        {
            vector<realtype> second1; //for storing the second1's from other processes
        ...
        vector<realtype> secondter( betas );
        ...
        {
            for(int l = 0; l < 2*index(k)+2; ++l)
            {
                term(l) = factor2(l) * factor1(2*index(k) + 1 - l); //tagal nagets to
            }

            for(int m = 2*index(k) + 2; m < d+1; ++m)
            {
                term1(m) = inner_prod( project(term, range(0,2*index(k)+2) ), project( factor, range(d-m , d - m + 2*index(k) + 2) ) ); //1/m!, 1/(m+1)!,...1/(m - 2k -1)
            }

            second( k ) = inner_prod( project ( constants, range (2*index(k) + 2, d+1) ), project ( term1 , range (2*index(k)+2, d+1) ) );
        }
        ...
        outfile1.open("/scratch3/chris.tica/SEXTIC/1_5k_mom_1_5kdig/results/SECOND.txt",std::ios_base::out);
        ...
        for ( i = 0; i < betas; ++i )
        {
            ...
            for (k = 0; k < index.size(); ++k)
            {
                total( index(k) ) = pow( -realtype(1)/beta(i), realtype( index(k) ) ) * second(k);
            }
        ...
        std::cout << duration.count() << "minutess" << std::endl;
    ...
    if ( world.rank() < LO  && world.rank() > 0  )
    {
        ...
        for (int k = 0; k < index.size(); ++k)
        {
            ...
            second1( k ) = inner_prod( project ( constants, range (2*index(k) + 2, d+1) ), project ( term1 , range (2*index(k)+2, d+1) ) );
        }

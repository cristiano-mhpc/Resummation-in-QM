...
int main()
{
    ...
    const int digits = 1500; //for setting the precision
    ...
    infile2.open("/scratch3/chris.tica/SEXTIC/construct_P_1_5kdig/matrix_p.txt", std::ios_base::in );
    for ( n = 0; n < d+1; n++ )
    {
        for ( m = 0; m < d+1; m++ )
        {
    ...
    for (i=0 ;i < d+1;i++)
    {
        outfile1 << scientific << constant(i)*mpfr::gamma(i+1)<< endl;
    }
    outfile1.close();

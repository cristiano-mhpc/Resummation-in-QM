...
int main()
{
    ...
    for (i = 0; i < betas-3; ++i)
    {
        beta(i) =  pow( realtype(10), realtype( i - 1 ) );
    }
    beta(betas-3) = realtype(1)/realtype(5);
    beta(betas-2) = realtype(4);
    beta(betas-1) = realtype(2) * pow( realtype(10), realtype(20) );

    //------------------read-in the first half of coefficients--------------------------------------
    std::string st;
    std::ifstream infile;
    infile.open( "/scratch1/scratch2/chris.tica/C++/moments_3k/moments.txt", std::ios_base::in );
    ...
    //------------------read-in the B(i)-----------------------------------------
    infile.open("/scratch3/chris.tica/C++/Pade/Constants/Constant.txt", std::ios_base::in);
    B(0) = realtype(1);
    std::string st1;
    for (i = 1; i < B.size(); ++i)
    {
    ...
    for ( i=0; i < betas; ++i )
    {
        ...
        outfile1 << std::scientific << realtype(1) + (beta(i) * pade(i)) << std::endl;
    }
